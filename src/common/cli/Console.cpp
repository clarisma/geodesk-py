...
void Console::finish(std::string_view msg)
{
	char buf[1024];
	char* p = putString(buf, "\u001b[97;48;5;64m");
	auto elapsed = std::chrono::steady_clock::now() - startTime_;
	int secs = std::chrono::duration_cast<std::chrono::seconds>(elapsed).count();
	p = Format::timeFast(p, secs, -1);
	p = putString(p, "\u001b[0m  ");
	size_t maxMsgChars = consoleWidth_ - 10;
	memset(p, ' ', maxMsgChars);
	memcpy(p, msg.data(), std::min(maxMsgChars, msg.size()));
	p[maxMsgChars] = '\n';
	size_t size = p + maxMsgChars + 1 - buf;
	print(buf, size);
}
